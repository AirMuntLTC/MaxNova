...
const VOICE_SPEECH_THRESHOLD = 18;
...
function stopVoiceSilenceDetection() {
  if (voiceSilenceTimer) {
    clearInterval(voiceSilenceTimer);
...
function startVoiceSilenceDetection() {
  stopVoiceSilenceDetection();

  if (!voiceAnalyser) return;

  const data = new Uint8Array(voiceAnalyser.fftSize);

  voiceSilenceTimer = setInterval(() => {
    if (!isVoiceRecording || !voiceAnalyser) return;
    ...
    if (level >= VOICE_SPEECH_THRESHOLD) {
    ...
    if (
      voiceSpeechDetected &&
      voiceLastLoudAt &&
      now - voiceLastLoudAt >= VOICE_SILENCE_MS &&
      now - voiceRecordingStartedAt >= VOICE_MIN_SPEECH_MS
    ) {
...
async function startVoiceRecording() {
  ...
  try {
    ...
    // Start conservative silence detection after the analyser is ready.
    setTimeout(() => {
      if (isVoiceRecording) startVoiceSilenceDetection();
    }, 250);
  } catch (error) {
    cleanupVoiceStream();
    resetVoiceRecordingState();
